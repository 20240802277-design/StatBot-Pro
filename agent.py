...
class CSVAnalystAgent:
    ...
    async def analyze(
        ...
        if (
            ...
            return self._demo_response(df, question, session_id)
    ...
    def _demo_response(self, df: pd.DataFrame, question: str, session_id: str) -> AnalysisResponse:
        """Return a rich demo response when no API key is configured."""
        rows, cols = df.shape
        col_names = list(df.columns)
        answer = (
            f"**Demo Mode** — No valid API key configured.\n\n"
            f"**Your question:** *\"{question}\"*\n\n"
            f"**Dataset loaded:** {rows:,} rows × {cols} columns\n"
            f"**Columns:** {', '.join(col_names)}\n\n"
            f"**To enable live AI analysis:**\n"
            f"1. Open `backend/.env`\n"
            f"2. Set a valid `OPENAI_API_KEY` (Novita, OpenAI, etc.)\n"
            f"3. The backend will auto-reload"
        )
        sample_col = col_names[0] if col_names else "column"
        code = (
            f"import pandas as pd\n"
            f"# df already loaded with {rows} rows\n"
            f"print(df['{sample_col}'].value_counts())"
        )
        return AnalysisResponse(
            session_id=session_id,
            status=AnalysisStatus.SUCCESS,
